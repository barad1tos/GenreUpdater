...
class TestSmartCacheConfig:
    ...
    def test_track_metadata_policy(self):
    ...
    def test_successful_api_metadata_policy(self):
    ...
    def test_failed_api_lookup_policy(self):
    ...
    def test_negative_result_policy(self):
    ...
    def test_should_use_fingerprint_validation(self):
    ...
    def test_should_use_event_invalidation(self):
    ...
    def test_is_persistent_cache(self):
    ...
    def test_ttl_formatting(self):
        """Test TTL formatting for human readability."""
        # Test various time formats
        assert "∞" in self.config._format_ttl(self.config.INFINITE_TTL)
        assert "1d" == self.config._format_ttl(24 * 60 * 60)
        assert "1h" == self.config._format_ttl(60 * 60) 
        assert "1m" == self.config._format_ttl(60)
        assert "30s" == self.config._format_ttl(30)
...
class TestCacheEvent:
    ...
    def test_track_event_creation(self):
        ...
        event = CacheEvent(
            event_type=CacheEventType.TRACK_ADDED,
            track_id="track123"
        )
    ...
    def test_track_event_requires_track_id(self):
    ...
    def test_fingerprint_event_creation(self):
        """Test creation of fingerprint change events."""
        event = CacheEvent(
            event_type=CacheEventType.FINGERPRINT_CHANGED,
    ...
    def test_fingerprint_event_requires_fingerprint(self):
    ...
    def test_library_sync_event(self):
...
class TestEventDrivenCacheManager:
    ...
    def test_should_invalidate_track_metadata(self):
        ...
        assert self.manager.should_invalidate_for_event(
            CacheContentType.TRACK_METADATA, event
        )
        ...
        assert not self.manager.should_invalidate_for_event(
            CacheContentType.TRACK_METADATA, event
        )
    
    def test_should_not_invalidate_time_based_content(self):
        ...
        assert not self.manager.should_invalidate_for_event(
            CacheContentType.FAILED_API_LOOKUP, event
        )
    
    def test_manual_invalidation_affects_all(self):
