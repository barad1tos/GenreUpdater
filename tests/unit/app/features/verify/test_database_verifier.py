...
class TestVerifyTrackExists:
    ...
    @pytest.mark.asyncio
    async def test_returns_true_on_error_assume_exists(
    ...
    @pytest.mark.asyncio
    async def test_converts_non_string_result_to_string(
        self, verifier: DatabaseVerifier
    ) -> None:
...
class TestShouldSkipVerification:
    ...
    async def test_returns_false_when_force(self, verifier: DatabaseVerifier) -> None:
        ...
        result = await verifier._should_skip_verification(
            force=True, csv_path="/any/path.csv", auto_verify_days=7
        )
        assert result is False
    ...
    async def test_returns_false_when_no_last_verify_file(
        ...
        result = await verifier._should_skip_verification(
            force=False, csv_path=csv_path, auto_verify_days=7
        )
        assert result is False
    ...
    async def test_returns_true_when_recently_verified(
        ...
        result = await verifier._should_skip_verification(
            force=False, csv_path=str(csv_file), auto_verify_days=7
        )
        assert result is True
...
class TestVerifyTracksInBatches:
    ...
    async def test_returns_invalid_track_ids(self, verifier: DatabaseVerifier) -> None:
        ...
        verifier.ap_client.run_script_code = AsyncMock(
            side_effect=["exists", "not_found", "exists"]
        )

        result = await verifier._verify_tracks_in_batches(
            tracks, {"batch_size": 10, "pause_seconds": 0}
        )
    ...
    async def test_processes_in_batches(self, verifier: DatabaseVerifier) -> None:
        ...
        with patch.object(verifier, "_log_batch_progress") as mock_log:
            await verifier._verify_tracks_in_batches(
                tracks, {"batch_size": 2, "pause_seconds": 0}
            )
...
class TestLogMethods:
    ...
    def test_log_batch_progress(
        self, verifier: DatabaseVerifier, caplog: pytest.LogCaptureFixture
    ) -> None:
        """Should log batch progress."""
        with caplog.at_level(logging.INFO):
            verifier._log_batch_progress(
                batch_num=1, total_batches=5, checked=20, valid=18, invalid=2
            )

        # Verify some progress was logged
        assert len(caplog.records) > 0

    def test_log_verify_complete(
        self, verifier: DatabaseVerifier, caplog: pytest.LogCaptureFixture
    ) -> None:
