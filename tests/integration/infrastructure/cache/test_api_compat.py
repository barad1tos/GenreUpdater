...
SNAPSHOT_PATH = Path("api_snapshots/cache_orchestrator_public_api.json")
MODULE_PATH = Path("src/infrastructure/cache/cache_orchestrator.py")
...
class APICompatibilityChecker:
    ...
    def check_signature_compatibility(self, name: str) -> dict[str, Any]:
        ...
        if validation_result := self._validate_compatibility_preconditions(name, api_data):
        ...
        try:
            ...
            checker_map = {"function": self._check_function_signature, "class": self._check_class_compatibility}
    ...
    def _check_function_signature(func: Any, snapshot_api: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            snapshot_required = sum(p.get("default") is None for p in snapshot_params.values())

            if current_required > snapshot_required:
                return {"compatible": False, "reason": f"More required parameters now ({current_required}) than before ({snapshot_required})"}
            ...
            return next(
                (
                    ...
                    if param_info.get("default") is None and param_name not in current_params
                ),
        ...
        try:
            ...
            if "__init__" in snapshot_methods:
                ...
                if not init_result["compatible"]:
                    return {"compatible": False, "reason": f"__init__ signature incompatible: {init_result['reason']}"}
...
def api_checker() -> APICompatibilityChecker:
    ...
    if not SNAPSHOT_PATH.exists():
        pytest.skip(f"API snapshot missing: {SNAPSHOT_PATH}")

    if not MODULE_PATH.exists():
        pytest.skip(f"Module path missing: {MODULE_PATH}")

    checker = APICompatibilityChecker(snapshot_path=str(SNAPSHOT_PATH), module_path=str(MODULE_PATH))
...
def test_api_snapshot_regeneration_instructions() -> None:
    ...
    print("  python tools/snapshot_api.py src/infrastructure/cache/cache_orchestrator.py api_snapshots/cache_orchestrator_public_api.json")
    print("\nTo view current API snapshot:")
    print("  cat api_snapshots/cache_orchestrator_public_api.json")
    print("\nTo compare before/after refactoring:")
    print("  git diff api_snapshots/cache_orchestrator_public_api.json")
    print("=" * 60)
