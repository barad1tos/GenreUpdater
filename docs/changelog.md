...
### Changed
- **Decompose `year_batch.py` god class (#218)**: extracted `PrereleaseHandler` (prerelease detection/handling) and `TrackUpdater` (track-level year updates with retry) into focused modules; `YearBatchProcessor` reduced from 1021 to ~530 LOC while preserving public API
