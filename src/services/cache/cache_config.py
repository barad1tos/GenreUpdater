...
class SmartCacheConfig:
    ...
    def should_use_fingerprint_validation(self, content_type: CacheContentType) -> bool:
        ...
        return policy.invalidation_strategy in [
            InvalidationStrategy.FINGERPRINT_BASED,
...
class CacheEvent:
    ...
    def __post_init__(self) -> None:
        """Validate event data."""
        if self.event_type in [CacheEventType.TRACK_ADDED, CacheEventType.TRACK_REMOVED,
                              CacheEventType.TRACK_MODIFIED] and not self.track_id:
    ...
    def emit_event(self, event: CacheEvent) -> None:
        ...
        for handler in handlers:
            try:
                ...
                self.logger.exception("Event handler failed: %s", e)

    def should_invalidate_for_event(self, content_type: CacheContentType,
                                   event: CacheEvent) -> bool:
        ...
        if content_type == CacheContentType.TRACK_METADATA:
            return event.event_type in [
                CacheEventType.TRACK_REMOVED,
                CacheEventType.TRACK_MODIFIED,
                CacheEventType.FINGERPRINT_CHANGED
            ]
        ...
        return (
            content_type == CacheContentType.ALBUM_YEAR
            and event.event_type == CacheEventType.LIBRARY_SYNC
        )
