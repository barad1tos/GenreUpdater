...
from src.utils.core.logger import ensure_directory
...
class AlbumCacheService:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger | None = None) -> None:
        ...
        # Cache file paths
        self.album_years_cache_file = Path(
            config.get("album_years_cache_file", "cache/album_years.csv")
        )
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(
            "AlbumCacheService initialized with %d albums",
    ...
    async def get_album_year(self, artist: str, album: str) -> str | None:
        ...
        if key in self.album_years_cache:
            ...
            self.logger.warning(
                "Hash collision detected for %s - %s, removing invalid entry",
                artist, album
            )
    ...
    async def store_album_year(self, artist: str, album: str, year: str) -> None:
        ...
        self.logger.debug(
            "Stored album year: %s - %s = %s",
            normalized_artist, normalized_album, normalized_year
        )
    ...
    async def save_to_disk(self) -> None:
        ...
        def blocking_save() -> None:
            ...
            try:
                ...
                self.logger.info(
                    "Album cache saved to %s (%d entries)",
                    self.album_years_cache_file, len(items)
                )
    ...
    def _read_csv_file(self) -> dict[str, tuple[str, str, str]]:
        ...
        try:
            ...
            self.logger.info(
                "Loaded %d album entries from %s",
                len(album_data), self.album_years_cache_file
            )

        except (OSError, UnicodeDecodeError, csv.Error) as e:
            self.logger.exception(
                "Error reading album cache file %s: %s",
                self.album_years_cache_file, e
            )
    ...
    def _process_csv_row(
        self, row: dict[str, str], album_data: dict[str, tuple[str, str, str]]
    ) -> None:
