...
class ApiCacheService:
    ...
    def _register_event_handlers(self) -> None:
        ...
        self.event_manager.register_event_handler(
            ...
            self._handle_track_removed
        )
        self.event_manager.register_event_handler(
            CacheEventType.TRACK_MODIFIED,
            self._handle_track_modified
        )
    ...
    async def set_cached_result(
        ...
        self.logger.debug(
            "Stored API result: %s - %s (%s) success=%s",
            artist, album, source, success
        )
    ...
    async def invalidate_for_album(self, artist: str, album: str) -> None:
        ...
        keys_to_remove.extend(
            ...
            if (
                cached_result.artist.lower().strip() == artist.lower().strip()
                and cached_result.album.lower().strip() == album.lower().strip()
            )
        )
    ...
    async def save_to_disk(self) -> None:
        ...
        def blocking_save() -> None:
            ...
            try:
                ...
                cache_data = {
                    key: serialize_model(result) for key, result in self.api_cache.items()
                }
