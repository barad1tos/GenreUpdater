...
class ErrorPatternDetector:
        ...
        if len(timestamps) >= MIN_TIMESTAMPS_FOR_PATTERN_DETECTION:
            ...
            older_count = len(
                [
                    ...
                    if error.timestamp - timedelta(minutes=20) <= ts < error.timestamp - timedelta(minutes=10)
                ]
            )
    ...
    def get_active_patterns(self, min_impact: float = 0.0) -> list[ErrorPattern]:
        ...
        active_patterns = [
            ...
            if pattern.last_seen >= cutoff_time and pattern.impact_score >= min_impact
        ]
...
class ErrorMetricsCollector:
    ...
    def record_error(
        ...
        if request.category is None or request.severity is None:
            classified_category, classified_severity = self.classifier.classify_error(
