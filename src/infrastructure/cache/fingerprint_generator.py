...
class FingerprintGenerator:
    ...
    def _extract_fingerprint_properties(self, track_data: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        fingerprint_data = {prop: track_data[prop] for prop in self.REQUIRED_PROPERTIES} | {
            prop: track_data.get(prop, default) for prop, default in self.OPTIONAL_PROPERTIES.items()
        }
    ...
    def _normalize_property_types(fingerprint_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            key: (
                FingerprintGenerator._normalize_numeric_value(value) if key in numeric_fields else FingerprintGenerator._normalize_string_value(value)
            )
